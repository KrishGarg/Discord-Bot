...
class HelpCommand(commands.Cog):
    ...
    **{0}help moderation :** To bring up the moderation command's help menu!
    ...
    **{0}say [text] :** Bot says whatever you said in place of [text]. Also if you don't want to double ping anyone, use '{{@}}' instead of '@' as I will replace {{@}} with @!
    ...
    **{0}warnings [user] :** Shows the warnings the mentioned person has received till now.
    ...
    ^ **Member should have the required permission to run the command!**
            '''.format(self.bot.command_prefix)
            the_embed = discord.Embed(title="↓ Moderation Commands Help Menu ↓",
                                      description=modhelp,
                                      color=0x00ff00)
            the_embed.set_author(name=ctx.message.author.name,
                                 icon_url=ctx.message.author.avatar_url)
